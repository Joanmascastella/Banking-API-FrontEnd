  ...
  <div class="content">
    ...
    <table id="transactions-table">
      ...
      <tbody>
        <tr v-for="(item, count) in transactions" :key="item.userId">
          <td>{{ ++count }}</td>
          <td>{{ item.date }}</td>
          <td v-if="item.fromAccount != 'ATM'">
            {{ item.fromAccount }} 
            <router-link :to="{ path: '#' }" class="link">
              <b-button v-b-tooltip.hover title="View account details">
                <img src="../../assets/img/account-details-icon.png">
              </b-button>
            </router-link>
          </td>
          <td v-else>{{ item.fromAccount }}</td>
          <td v-if="item.toAccount != 'ATM'">
            {{ item.toAccount }} 
            <router-link :to="{ path: '#' }" class="link">
              <b-button v-b-tooltip.hover title="View account details">
                <img src="../../assets/img/account-details-icon.png">
    ...
    <div id="report-container">
      ...
      <div>
        ...
        <div class="report-data">
          <h2>€{{ report.minimumAmount }}</h2>
        </div>
      </div>
      <div>
        <h2>Maximum amount</h2>
        <div class="report-data">
          <h2>€{{ report.maximumAmount }}</h2>
        </div>
      </div>
      <div>
        <h2>Total amount</h2>
        <div class="report-data">
          <h2>€{{ report.totalAmount }}</h2>
        </div>
      </div>
    </div>
    <b-button v-b-tooltip.hover title="View all transactions" id="report-link" @click="viewAllTransactions">
      <img id="transaction-list" src="../../assets/img/transactions.png">
    </b-button>
    <b-button v-b-tooltip.hover title="View transaction report" id="report-link" @click="viewReport">
      <img id="transaction-report" src="../../assets/img/transaction-report-icon.png">
    </b-button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    updateReport() {
      ...
      this.report.count = this.transactions.length;
      this.report.minimumAmount = amounts.length ? Math.min(...amounts) : 0;
      this.report.maximumAmount = amounts.length ? Math.max(...amounts) : 0;
      this.report.totalAmount = amounts.reduce((sum, amount) => sum + amount, 0);
    },
